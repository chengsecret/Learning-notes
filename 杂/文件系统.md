...
- **unsigned char *myvhard;**    //初始化为内存大小 1024000

- **unsigned char *blockaddr[BLOCKNUM];**  //分别指向每一个盘块的位置

  

- **useropen openfilelist[MAXOPENFILE]; ** //topenfile=0，都未被占用
...
void startsys() {
  ...
  if (!need_format) {
    ...
    printf("FileSystem not exit, now create it!\n");
    my_format();
  }
